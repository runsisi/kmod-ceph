...
#define __ceph_bio_iter_advance_step(it, n, STEP) do {			      \
	...
	while (__n) {							      \
		...
		if (!(it)->iter.bi_size && (it)->bio->bi_next) {	      \
			...
			bi_iter.bi_sector = (it)->bio->bi_sector;   \
			bi_iter.bi_size = (it)->bio->bi_size;       \
			bi_iter.bi_idx = (it)->bio->bi_idx;         \
			bi_iter.bi_bvec_done = 0;                   \
			(it)->iter = bi_iter;		      \
		}							      \
		__n -= __cur_n;						      \
	}								      \
...
#define ceph_bio_iter_advance_step(it, n, BVEC_STEP)			      \
	__ceph_bio_iter_advance_step(it, n, ({				      \
		...
		__kc_bio_for_each_segment(bv, (it)->bio, __cur_iter, __cur_iter) \
			(void)(BVEC_STEP);				      \
	}))
